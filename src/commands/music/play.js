...
const { connect, ConnectionStatus, createResource } = require('../../objects/subscription.js');
		...
		if (!ytdl.validateURL(url)) {
			...
			switch (status) {
				case QueueVideoStatus.SUCCESS:
					if (playNow) {
						...
						if (queue.length >= 1) {
							...
							const saveStatus = await setData(interaction.guildId, 'queue', queue);
							if (saveStatus) {
								...
								if (resource != null) {
									...
									embed = successEmbed('Playing the music now.', 'Enjoy the music.');
								} else {
									embed = errorEmbed('Unknown error occurred!', 'A problem that the developer do not know wtf just happened.');
								}
							} else {
						...
						embed = successEmbed('Queued the music.', 'The song was placed at the end of the queue.');
					}
					break;
				...
				case QueueVideoStatus.ERROR_ALREADY_EXIST:
					if (playNow) {
						let queue = await retrieveData(interaction.guildId, 'queue');
						const videoInfo = await ytdl.getBasicInfo(url, []);

						const index = queue.findIndex((element) => element['url'] === videoInfo.videoDetails.video_url);
						const indexValue = queue[index];
						if (index == 0) {
							embed = errorEmbed('The required music is currently playing!', 'Did you not noticing that?');
						} else if (index != -1) {
							const firstItem = queue.shift();
							queue.push(firstItem);
							queue = queue.filter(element => element !== indexValue);
							...
							if (saveStatus) {
								if (queue.length >= 1) {
									...
									if (resource != null) {
										player.play(resource);
										embed = successEmbed('Moved the required music to the front!', `Since the music was already in the queue, Smoothie moved the required music from ${index + 1} to the front.`);
									} else {
										embed = errorEmbed('Unknown error occurred!', 'A problem that the developer do not know wtf just happened.');
									}
								}
